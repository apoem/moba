module.exports = (app) => {
  ...
  //     name: '新闻分类'
  //   })
  //   const categories = await Category.find().where({ parent })
  //   const newsTitle = ["体验服爆料丨拒绝被抢野，loading能打气，峡谷调整抢先知", "全新限时活动玩法【万镜觉醒】3.13开启，五倍快乐等你来~", "王者音乐课：五岳山河志", "【虎牙大腿杯S3】玩法升级，火热开战！", "体验服爆料丨全新玩法万镜觉醒，爽快翻5倍！", "3月13日体验服不停机更新公告", "【体验服】王者模拟战模式暂时关闭调优公告", "3月13日体验服停机更新公告", "心愿单功能异常说明", "3月13日全服不停机更新公告", "白色情人节【玫瑰芬芳，纸短情长】活动公告", "白色情人节 浪漫好礼来袭", "峡谷女神节 福利大集结", "【稷下修学游】活动公告", "伽罗星元上新 多重福利来袭", "王者荣耀国际巡回赛（KPLGT）2020年春季赛开赛时间公告", "2020年KPL春季赛大名单公布", "2020年KPL春季赛“云开赛”在即 全新视觉与六大城市主场来袭！", "高校联赛女神挑战赛周末开赛啦！来看高颜值组合强强联手", "你和那个TA会是峡谷校园最佳CP吗？高校联赛白色情人节开启CP大作战！"];
  //   const newsList = newsTitle.map((title) => {
  //     const categoriesRandom = categories.slice(0).sort((a, b) => Math.random() - 0.5)
  //     return {
  //       name: title,
  //       categories: categoriesRandom.slice(0, 2)
  //     }
  //   })
  ...
  router.get('/news/list', async (req, res) => {
    const parent = await Category.find({ name: '新闻分类' })
    const list = await Category
      .aggregate([
        {
          $match: { parent: parent[0]._id }
        },
        {
          $lookup: {
            from: 'articles',
  ...
  //       ))
  //        Hero.insertMany(item.heroes)        
  //     }
  //     res.send(await Hero.find().populate('categories'))
  // })


  router.get('/heroes/list', async (req, res) => {
    const parent = await Category.find({ name: '英雄分类' })
    const list = await Category
      .aggregate([
        {
          $match: { parent: parent[0]._id }
    ...
    list.unshift({
      name: '热门',
      heroes: await Hero.find().populate('categories').limit(6).lean()
    })
    res.send(list)
  })
  ...
  router.get('/article/:id', async (req, res) => {
    ...
    const text = await Article.find({ categories: { $in: data.categories } }).limit(3).lean()
    data.related = text.filter((el) => { return (el.name != data.name) })
    res.send(data)
  })
